function Get-AAPCachedJobTemplates {
    ...
    .DESCRIPTION
        Fetches all job templates from /api/controller/v2/job_templates/ with pagination.
    ...
    $path = '/api/controller/v2/job_templates/?page_size=200'
    ...
    while ($path) {
        ...
        if ($response.next) {
            # next is a full path like /api/controller/v2/job_templates/?page=2
            $path = $response.next
        } else {
            $path = $null
        }
